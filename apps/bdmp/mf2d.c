...
int main(int argc, char **argv)
{
  ...
  if (argc != 8) {
    if (params->mype == 0)
      fprintf(stderr, "Usage: %s filename nrowpes ncolpes niters nfactors type[1:FR;2:RR] mlock[0;1]\n", argv[0]);
    BDMPI_Finalize();
    return EXIT_FAILURE;
  }
  ...
  params->mlock    = atoi(argv[7]);
  params->lrnrate  = 0.001;
  params->alpha    = 0.01;
  params->lambda   = 0.01;

  printf("[%3d] grid: %dx%d, niters: %d, nfactors: %d, type: %d, mlock: %d\n",
      params->mype, params->nrowpes, params->ncolpes, params->niters, 
      params->nfactors, params->type, params->mlock);
